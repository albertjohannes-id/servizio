...
- Auth: **local only**. One profile per device — email + 6-digit PIN (PBKDF2 hash + salt in `servizio_v1_auth`). Unlock session `servizio_v1_unlock` expires after 1 hour idle. Logout = lock (PIN again). After 10 wrong PINs, user may reset device (wipes local data). JSON backup/export on Account screen; photos not included in backup. Archive is soft-hide; Account → Archived assets can restore or permanently delete (also removes that asset’s logs/changes).
